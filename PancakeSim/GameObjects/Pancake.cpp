...
void Pancake::Update(float force, int panSpriteId, int panRotation, float onHobValue, float flameSize)
{
	...
	{ 	//TODO: replace magic numbers throughtout this if statment
		...
		{
			float fixed_x_pos = startPosition + ((float)panRotation * 0.85f) - (( ( panRotation < 0.0f ? -1.0f : 1.0f ) * 6.0f));
	...
	else if ( GetPosition()->y >= panBottomPosition - (panBottom_spriteIdOffset * panSpriteId) - offHobOffset - 60 && rigidbody->GetVelocity()->y < 0)	//correct position when entering pan
	{
		spriteID = (spriteSheet->GetTotalSprites() - 1) - floor(panSpriteId / 2.0f);
		SetRotation(panRotation);
